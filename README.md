      ...
      <div className="relative mx-auto flex w-full max-w-6xl flex-col gap-16 px-6 pb-24 pt-16 sm:px-10">
        ...
        <section className="grid gap-10 lg:grid-cols-[1.4fr_1fr]">
          ...
          <aside className="space-y-8">
            ...
            {service.miniFaq?.length ? (
              <div className="rounded-[2.5rem] border border-white/10 bg-white/5 p-8 text-sm text-slate-200/80">
                <p className="text-xs uppercase tracking-[0.35em] text-cyan-200/70 visual-accent-text">Mini-FAQ</p>
                <dl className="mt-4 space-y-4">
                  {service.miniFaq.map((item) => (
                    <div key={item.question} className="space-y-1">
                      <dt className="text-sm font-semibold text-white">{item.question}</dt>
                      <dd className="text-sm text-white/75">{item.answer}</dd>
                    </div>
                  ))}
                </dl>
              </div>
